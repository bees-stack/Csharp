...
namespace StudentManage
{
    public partial class Login : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("输入账号密码验证码？", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else if (id == "")
            {
                MessageBox.Show("账号不能为空啊", "警告：", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            else if (paw == "")
            {
                MessageBox.Show("密码不能为空啊", "警告：", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            else
            {
                ...
                else
                {
                    Dao dao = new Dao();
                        ...
                        {
                            string sqlstr = string.Format("select * from guess  where ID='{0}' and Paw='{1}'", id, paw);
                            DataTable dt = dao.GetDataTable(sqlstr);
                                //返回受影响行数>0即为登录成功
                                if (dt != null && dt.Rows.Count > 0)
                                {
